...
interface LibrarySessionContextValue {
  ...
  forceLogoutUser: (userId: string) => Promise<boolean>;
...
export function LibrarySessionProvider({ children }: { children: ReactNode }) {
  ...
  // Realtime: update session state on backend events
  useEffect(() => {
    let disconnect: (() => void) | null = null;
    (async () => {
      const { connectRealtime } = await import("@/services/backendRealtime");
      disconnect = await connectRealtime((ev, payload) => {
        if (ev === 'session_update') {
          // If our user logged out from another client, clear session
          if (payload?.type === 'logout' && payload?.userId && payload.userId === session?.userId) {
            setSession(null);
          }
        }
      }, { userId: user?.id });
