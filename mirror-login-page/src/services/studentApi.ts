...
class StudentApiService {
  private readonly BASE_URL = `${API.BACKEND.BASE}/api`;
  ...
  async registerStudent(data: StudentRegistrationData): Promise<{ success: boolean; message: string; studentId?: string }> {
    try {
      ...
      const response = await fetch(`${this.BASE_URL}/students/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          // Map registration data to Python backend format
  ...
  async updateStudentProfile(studentId: string, data: StudentProfileUpdateData): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const response = await fetch(`${this.BASE_URL}/students/${studentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
