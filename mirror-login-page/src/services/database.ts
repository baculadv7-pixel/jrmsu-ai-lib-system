...
class DatabaseService {
  ...
  authenticateWithQRCode(qrData: any): { success: boolean; user?: User; error?: string } {
    ...
    // FLEXIBLE NAME MATCHING: Allow authentication even if name changed
    // Primary authentication is by userId, name is secondary verification
    if (qrName !== dbName) {
      console.warn('Full name mismatch detected (QR may be outdated):', {
        qrName: qrData.fullName,
        dbName: user.fullName,
        qrNameNormalized: qrName,
        dbNameNormalized: dbName
      });
      console.warn('Proceeding with authentication based on userId match');
      console.warn('User should regenerate QR code to update name');
      // Don't fail authentication - user ID match is sufficient
    ...
    this.logLogin({
      userId: user.id,
      method: "QR_CODE", // QR code login method
      success: true,
      timestamp: new Date(),
      userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : 'unknown'
    });
  ...
  // Auto-provision from QR data (dev convenience)
  upsertUserFromQR(qr: any): { success: boolean; user?: User; error?: string } {
    try {
      const id = (qr?.userId || '').toString().trim();
      const userType = (qr?.userType || '').toString().trim().toLowerCase();
      const fullName = (qr?.fullName || '').toString().trim();
      if (!id || !fullName || (userType !== 'admin' && userType !== 'student')) {
        return { success: false, error: 'Invalid QR payload for provisioning' };
      }
      ...
      const email = `${id.replace(/[^a-zA-Z0-9]/g,'').toLowerCase()}@local.dev`;
      if (existing) {
        // Update minimal fields + regenerate QR
        const res = this.updateUser(id, {
          fullName,
        ...
        return this.createUser({
          ...
          lastName,
          email,
          userType: userType as 'admin' | 'student',
  ...
  updateUser(id: string, updates: Partial<User>): { success: boolean; user?: User; error?: string } {
    ...
    // Auto-regenerate QR code if name-related fields changed
    const nameFieldsChanged = 
      (updates.firstName && updates.firstName !== oldUser.firstName) ||
      (updates.middleName && updates.middleName !== oldUser.middleName) ||
      (updates.lastName && updates.lastName !== oldUser.lastName) ||
      (updates.suffix && updates.suffix !== oldUser.suffix) ||
      (updates.fullName && updates.fullName !== oldUser.fullName);
    
    if (nameFieldsChanged) {
      console.log('🔄 Name changed - auto-regenerating QR code for user:', id);
      // Regenerate QR code with updated name
      const qr = this.buildStandardUserQR(updatedUser);
      updatedUser.qrCodeData = JSON.stringify(qr);
      updatedUser.qrCodeGeneratedAt = new Date();
      console.log('✅ QR code regenerated with new name:', updatedUser.fullName);
    }
