...
export function QRScanner({ onScanSuccess, onError }: QRScannerProps) {
  ...
  // Dynamic zoom control to handle too-near / too-far cases
  ...
  const drawOverlay = (loc: any) => {
    ...
    const cur = {
      tl: { x: loc.topLeftCorner.x, y: loc.topLeftCorner.y },
      tr: { x: loc.topRightCorner.x, y: loc.topRightCorner.y },
      br: { x: loc.bottomRightCorner.x, y: loc.bottomRightCorner.y },
      bl: { x: loc.bottomLeftCorner.x, y: loc.bottomLeftCorner.y },
    };
    let sm = cur;
    if (prevCornersRef.current) {
      sm = {
        tl: { x: lerp(prevCornersRef.current.tl.x, cur.tl.x), y: lerp(prevCornersRef.current.tl.y, cur.tl.y) },
        tr: { x: lerp(prevCornersRef.current.tr.x, cur.tr.x), y: lerp(prevCornersRef.current.tr.y, cur.tr.y) },
        br: { x: lerp(prevCornersRef.current.br.x, cur.br.x), y: lerp(prevCornersRef.current.br.y, cur.br.y) },
        bl: { x: lerp(prevCornersRef.current.bl.x, cur.bl.x), y: lerp(prevCornersRef.current.bl.y, cur.bl.y) },
      };
    }
    prevCornersRef.current = sm;

    // Clear and draw quad with glow
    ...
    ctx.shadowBlur = 12;
    ctx.beginPath();
    ctx.moveTo(sm.tl.x, sm.tl.y);
    ctx.lineTo(sm.tr.x, sm.tr.y);
    ctx.lineTo(sm.br.x, sm.br.y);
    ctx.lineTo(sm.bl.x, sm.bl.y);
    ctx.closePath();
    ctx.stroke();

    const tick = 18;
    const drawTick = (p: any, dx: number, dy: number) => { ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(p.x + dx * tick, p.y + dy * tick); ctx.stroke(); };
    drawTick(sm.tl, 1, 0); drawTick(sm.tl, 0, 1);
    drawTick(sm.tr, -1, 0); drawTick(sm.tr, 0, 1);
    drawTick(sm.br, -1, 0); drawTick(sm.br, 0, -1);
    drawTick(sm.bl, 1, 0); drawTick(sm.bl, 0, -1);

    const cx = (sm.tl.x + sm.br.x) / 2; const cy = (sm.tl.y + sm.br.y) / 2;
    ctx.fillStyle = 'rgba(16,185,129,0.9)'; ctx.beginPath(); ctx.arc(cx, cy, 5, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  };
  ...
  const logDiagnostics = (info: Partial<DiagnosticsInfo>) => {
    const diagnosticsInfo: DiagnosticsInfo = {
      ...
      containerVisible: isContainerVisible(containerRef.current || undefined),
      lastError: error,
  ...
  const isContainerVisible = (el?: HTMLElement): boolean => {
  ...
  const startScanner = async (cameraId?: string, attemptNumber: number = 1): Promise<void> => {
    ...
    try {
      ...
      const scannerConfig = {
        fps: 24, // higher FPS improves moving/handshake detection
        // qrbox omitted to scan full frame for far/angled codes
        aspectRatio: 1.0,
        disableFlip: false,
      ...
      // Force video element to be visible and properly sized and enhance camera controls (focus/exposure/torch)
      setTimeout(() => {
        (async () => {
          const container = (containerRef.current as HTMLElement | null) || (document.getElementById('qr-scanner-container') as HTMLElement | null);
          if (container) {
            ...
            if (videoElement) {
              ...
              // Try to enhance active video track for low/high light and motion
              try {
                const stream = (videoElement as any).srcObject as MediaStream | undefined;
                const track = stream?.getVideoTracks()?.[0];
                if (track) {
                  const caps: any = track.getCapabilities?.() || {};
                  const cons: any = track.getConstraints?.() || {};
                  const advanced: any[] = [];
                  ...
                  if (caps.frameRate?.max) advanced.push({ frameRate: Math.min(30, caps.frameRate.max) });
                  const apply = Object.keys(advanced).length ? { advanced } : {};
                  await track.applyConstraints({ ...cons, ...apply });
                  console.log('🎛️ Applied camera constraints', apply);
                }
              } catch (e) {
              ...
              // Initialize mid zoom if supported
              try {
                const stream = (videoElement as any).srcObject as MediaStream | undefined;
                const track = stream?.getVideoTracks()?.[0];
                const caps: any = track?.getCapabilities?.();
                if (track && caps?.zoom && lastZoomRef.current == null) {
                  const mid = (caps.zoom.min + caps.zoom.max) / 2;
                  await track.applyConstraints({ advanced: [{ zoom: mid }] });
                  lastZoomRef.current = mid;
                }
              } catch {}
            } else {
      ...
      const jsQRDetection = setInterval(() => {
        ...
        try {
          // Always clear overlay to avoid accumulating black tint
          ...
          if (video && video.videoWidth > 0 && video.videoHeight > 0) {
            ...
            if (ctx) {
              ...
              // Simple contrast boost to aid low-light/washed images
              try {
                ...
                for (let i = 0; i < d.length; i += 4) {
                  ...
                  y = Math.max(0, Math.min(255, (y - 128) * factor + 128));
                  d[i] = d[i+1] = d[i+2] = y;
                }
              } catch {}
              ...
              const containerEl = (containerRef.current as HTMLElement | null) || (document.getElementById('qr-scanner-container') as HTMLElement | null);
              if (overlay && containerEl && qrResult?.location) {
                ...
                try {
                  ...
                  const track = stream?.getVideoTracks()?.[0];
                  const caps: any = track?.getCapabilities?.();
                  if (track && caps?.zoom && Date.now() > zoomCooldownRef.current) {
                    const sizeRatio = Math.max(boxW / (overlay.width || 1), boxH / (overlay.height || 1));
                    let target = lastZoomRef.current ?? ((caps.zoom.min + caps.zoom.max) / 2);
                    if (sizeRatio < 0.08) { // too far
                      target = Math.min(caps.zoom.max, (lastZoomRef.current ?? target) * 1.25);
                    } else if (sizeRatio > 0.65) { // too near
                      target = Math.max(caps.zoom.min, (lastZoomRef.current ?? target) * 0.8);
                    } else {
                      target = null as any;
                    }
                    if (target != null && Math.abs((lastZoomRef.current ?? 0) - target) > 0.05) {
                      await track.applyConstraints({ advanced: [{ zoom: target }] });
                      lastZoomRef.current = target;
                      zoomCooldownRef.current = Date.now() + 600; // throttle
                    }
                  }
                } catch {}
              }
            ...
            // If no QR detected, keep overlay clear so camera view doesn't darken
          } catch (error) {
            console.debug('jsQR detection failed:', error);
          }
        })().catch((error) => { /* swallow interval errors */ console.debug('jsQR tick error:', error); });
        }, 60); // faster updates for smoother tracking
