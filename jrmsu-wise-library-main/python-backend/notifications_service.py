...
class NotificationsService:
    ...
    def create_activity_log(
        ...
        """
        with get_db_cursor() as cursor:
            # Map to existing activity_log schema: actor_id, actor_name, event, details, source, timestamp
            payload = details or {}
            if summary:
                try:
                    # include summary in details JSON
                    if isinstance(payload, dict):
                        payload = {**payload, 'summary': summary}
                    else:
                        payload = {'summary': summary, 'details': str(payload)}
                except Exception:
                    payload = {'summary': summary}
            cursor.execute("""
                INSERT INTO activity_log (actor_id, actor_name, event, details, source, timestamp)
