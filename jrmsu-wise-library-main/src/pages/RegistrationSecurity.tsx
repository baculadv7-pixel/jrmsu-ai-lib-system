...
const RegistrationSecurity = () => {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Wait 2 seconds then redirect (prefer returnTo if provided by mirror page)
      setTimeout(() => {
        reset();
        let rt: string | null = null;
        try { rt = sessionStorage.getItem('mirror_return_to'); } catch {}
        if (rt) {
          try { sessionStorage.removeItem('mirror_return_to'); } catch {}
          // Use full page navigation to cross-origin mirror
