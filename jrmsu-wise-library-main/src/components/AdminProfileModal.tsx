...
useEffect(() => {
  ...
  const handleRegenerateQR = async () => {
    try {
      // Generate new QR code with current admin data
      const qrData = {
        ...
        systemTag: formData.systemTag || 'JRMSU-KCL',
        timestamp: Date.now(),
        encryptedPasswordToken: btoa(`${formData.id}-${Date.now()}`),
        email: formData.email,
        position: formData.position,
        role: 'Administrator'
      };
      
      // Update admin with new QR code data in localStorage
      const updateResult = databaseService.updateUser(formData.id, {
      ...
      // Update form data with new QR info
      setFormData(prev => ({
        ...prev,
        qrCodeData: JSON.stringify(qrData),
        qrCodeGeneratedAt: new Date()
