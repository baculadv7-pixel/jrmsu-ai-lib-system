...
export function StudentProfileModal({ isOpen, onClose, student, onSave }: StudentProfileModalProps) {
  ...
  const handleRegenerateQR = useCallback(async () => {
    ...
    try {
      // Generate new QR code with current student data
      const qrData = {
        ...
        timestamp: Date.now(),
        encryptedPasswordToken: btoa(`${student.id}-${Date.now()}`),
        email: student.email,
      ...
      // Update student with new QR code data in localStorage
      const updateResult = databaseService.updateUser(student.id, {
        qrCodeData: JSON.stringify(qrData),
        qrCodeGeneratedAt: new Date()
      });
      
      if (!updateResult.success) {
        throw new Error(updateResult.error || "Failed to regenerate QR code");
      }
      
      // Sync to MySQL database for global persistence
      await databaseService.syncQRCodeToBackend(
        student.id,
        JSON.stringify(qrData),
        'student'
      );
      
      // Update local state with new QR data
      onSave(updateResult.user!);
      
      toast({
        title: "QR Code regenerated",
        description: "A new QR code has been generated and synced to the database.",
      });
    } catch (error) {
