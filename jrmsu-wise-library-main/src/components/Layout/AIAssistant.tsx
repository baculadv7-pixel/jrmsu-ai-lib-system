...
const AIAssistant = () => {
  ...
  // Auto-open for System Guide (toggle). Study Helper only opens on book-selected event.
  useEffect(() => {
    (async () => {
      try {
        if (!user?.id) return;
        const prefs = PreferenceService.load(user.id);
        if (prefs.aiGuideEnabled) {
          setIsOpen(true);
          const pageKey = window.location.pathname || 'current page';
          try {
            const guide = await aiService.getSystemGuide(pageKey, user.id);
            const msg: ChatMessage = { id: `msg_${Date.now()}_guide`, role: 'assistant', content: guide, timestamp: new Date(), userId: user.id };
  ...
  const speak = useCallback((text: string) => {
    try {
      ...
      const prefs = user?.id ? PreferenceService.load(user.id) : {} as any;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Speak final assistant message if enabled
      speak((response as any).content || "");
    } catch (error) {
  ...
  // Listen for Assistant Mode triggers (global events)
  useEffect(() => {
    if (!user?.id) return;

    const openAiHandler = () => setIsOpen(true);

    const systemGuideHandler = async () => {
      setIsOpen(true);
      try {
        const pageKey = window.location.pathname || 'current page';
        const guide = await aiService.getSystemGuide(pageKey, user.id!);
    ...
    const bookSelectedHandler = async (ev: Event) => {
      const detail = (ev as CustomEvent).detail || {};
      if (!detail.book) return;
      setIsOpen(true);
      try {
        const content = await aiService.getBookOverview(detail.book, user.id!);
        const msg: ChatMessage = { id: `msg_${Date.now()}_book`, role: 'assistant', content, timestamp: new Date(), userId: user.id! };
        setMessages(prev => [...prev, msg]);
      } catch { /* noop */ }
    };
    ...
    const openSupportHandler = () => {
      setIsOpen(true);
      const support = `Developer Support\n\nTeam: CS-3\nName: Jhon Mark A. Suico\nEmail: suicojm99@gmail.com\nContact: 0946-886-1715\n\nFor technical issues, please include screenshots, steps to reproduce, and your user ID: ${user.id}.`;
      const msg: ChatMessage = { id: `msg_${Date.now()}_support`, role: 'assistant', content: support, timestamp: new Date(), userId: user.id! };
      setMessages(prev => [...prev, msg]);
    };

    window.addEventListener('jrmsu:open-ai', openAiHandler as any);
    window.addEventListener('jrmsu:system-guide', systemGuideHandler as any);
    window.addEventListener('jrmsu:book-selected', bookSelectedHandler as any);
    window.addEventListener('jrmsu:open-support', openSupportHandler as any);
    return () => {
      window.removeEventListener('jrmsu:open-ai', openAiHandler as any);
      window.removeEventListener('jrmsu:system-guide', systemGuideHandler as any);
      window.removeEventListener('jrmsu:book-selected', bookSelectedHandler as any);
      window.removeEventListener('jrmsu:open-support', openSupportHandler as any);
    };
  }, [user?.id]);
