...
export const pythonApi = {
  ...
  async updateNotificationPreferences(params: { userId: string; emailNotifications: boolean; smsReminders: boolean; pushNotifications: boolean }) {
    try {
      const r = await fetch(`${BASE}/api/users/${encodeURIComponent(params.userId)}/notification-preferences`, {
        method: 'POST',
        ...json({
          emailNotifications: params.emailNotifications,
          smsReminders: params.smsReminders,
          pushNotifications: params.pushNotifications
        })
      });
      if (!r.ok) {
        const errorData = await r.json().catch(() => ({ message: r.statusText }));
        return { success: false, message: errorData.message || 'Failed to update notification preferences' };
      }
      const data = await r.json();
      return { success: true, ...data };
    } catch (error: any) {
