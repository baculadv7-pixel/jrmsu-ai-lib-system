...
class DatabaseService {
  ...
  authenticateWithQRCode(qrData: any): { success: boolean; user?: User; error?: string } {
    ...
    this.logLogin({
      userId: user.id,
      method: "QR_CODE", // QR code authentication
      success: true,
      timestamp: new Date(),
      userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : 'unknown'
    });
  ...
  createUser(userData: Partial<User>): { success: boolean; user?: User; error?: string } {
    ...
    // Sync QR code to MySQL database via backend API
    this.syncQRCodeToBackend(newUser.id, newUser.qrCodeData, newUser.userType).then(result => {
      if (!result.success) {
        console.error('Failed to sync QR code to MySQL:', result.error);
      }
    });
  ...
  async syncQRCodeToBackend(userId: string, qrCodeData: string | undefined, userType: 'student' | 'admin'): Promise<{ success: boolean; error?: string }> {
    try {
      if (!qrCodeData) {
        return { success: false, error: 'No QR code data to sync' };
      }

      const endpoint = userType === 'student' 
        ? `http://localhost:5000/api/students/${encodeURIComponent(userId)}/qr-code`
        : `http://localhost:5000/api/admins/${encodeURIComponent(userId)}/qr-code`;
      ...
      console.log('✅ QR code synced to MySQL:', userId);
      return { success: true };
    } catch (error: any) {
      console.error('❌ Failed to sync QR code to backend:', error);
      return { 
        success: false, 
  ...
  // Fetch QR code from backend MySQL database
  async fetchQRCodeFromBackend(userId: string, userType: 'student' | 'admin'): Promise<{ success: boolean; qrCodeData?: string; error?: string }> {
    try {
      const endpoint = userType === 'student'
        ? `http://localhost:5000/api/students/${encodeURIComponent(userId)}/qr-code`
        : `http://localhost:5000/api/admins/${encodeURIComponent(userId)}/qr-code`;
      ...
      const result = await response.json();
      console.log('✅ QR code fetched from MySQL:', userId);
      return { 
        success: true, 
