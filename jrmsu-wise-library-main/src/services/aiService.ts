...
class AIService {
  ...
  // Placeholder: ensure auth token when using backend proxy
  // Currently a no-op to avoid runtime errors; extend to actually attach tokens if needed
  async ensureAuthToken(userId: string, role: string): Promise<void> {
    return;
  }
  ...
  // Assistant Mode: Study Helper - generate book overview and key knowledge
  async getBookOverview(book: { title: string; author?: string; isbn?: string; category?: string; summary?: string }, userId: string): Promise<string> {
    const parts = [
      `Title: ${book.title}`,
      book.author ? `Author: ${book.author}` : '',
      book.isbn ? `ISBN: ${book.isbn}` : '',
      book.category ? `Category: ${book.category}` : '',
      book.summary ? `Known summary: ${book.summary}` : ''
    ].filter(Boolean).join('\n');

    const prompt = `Provide a concise, student-friendly overview for this book and what a student can learn from it. Focus on key topics, concepts, and how it relates to studies. Use bullet points when helpful.\n\n${parts}`;
    const response = await this.sendMessage(prompt, userId, []);
    return response.content;
  }

  // Assistant Mode: System Guide - page-specific tutorial
  async getSystemGuide(pageKey: string, userId: string): Promise<string> {
    const prompt = `You are Jose, the JRMSU library system guide. Explain step-by-step how to use the "${pageKey}" page: where to click, what each key control does, and the typical workflow. Keep it short and actionable.`;
    const response = await this.sendMessage(prompt, userId, []);
    return response.content;
  }
