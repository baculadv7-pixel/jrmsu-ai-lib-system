...
REM ---- AGGRESSIVE PORT CLEANUP ----
echo [Cleanup] Killing all Node.js processes...
taskkill /F /IM node.exe 2>nul

REM ---- Free ports (Ollama 11434, Backend 5000, Frontend 8080) ----
echo [Cleanup] Freeing required ports...
call :kill_port 11434 Ollama
call :kill_port 5000 Backend
call :kill_port 5001 Backend-Alt
call :kill_port 8080 Frontend
call :kill_port 8081 Mirror

echo [Cleanup] Waiting 3 seconds for ports to release...
...
REM -------- Start Python backend (port 5000 - FORCED) --------
if exist "%PY_BACKEND%\app.py" (
  if exist "%PY_BACKEND%\.venv\Scripts\activate.bat" (
    echo [Backend] Starting Python server with venv on http://localhost:5000 ...
    start "JRMSU Backend (Port 5000)" cmd /K "cd /D \"%PY_BACKEND%\" && call .venv\Scripts\activate.bat && python app.py"
  ) else (
    echo [Backend] Starting Python server on http://localhost:5000 ...
    start "JRMSU Backend (Port 5000)" cmd /K "cd /D \"%PY_BACKEND%\" && python app.py"
  )
  echo [Backend] Waiting 8 seconds for backend initialization...
  timeout /t 8 /nobreak > nul
  echo [Backend] Backend should be ready now.
) else (
...
echo [Frontend] Starting Vite on http://localhost:%DEV_PORT% (FORCED) ...
start "JRMSU Frontend (Port 8080)" cmd /K "cd /D \"%ROOT%\" && set VITE_API_BASE=%VITE_API_BASE% && set VITE_AUTH_API_BASE=%VITE_AUTH_API_BASE% && set VITE_AUTH_REQUEST_RESET_PATH=%VITE_AUTH_REQUEST_RESET_PATH% && set VITE_AUTH_VERIFY_CODE_PATH=%VITE_AUTH_VERIFY_CODE_PATH% && set VITE_AUTH_RESET_PASSWORD_PATH=%VITE_AUTH_RESET_PASSWORD_PATH% && set VITE_AI_API_BASE=%VITE_AI_API_BASE% && set VITE_AI_CHAT_PATH=%VITE_AI_CHAT_PATH% && set VITE_AI_HEALTH_PATH=%VITE_AI_HEALTH_PATH% && npm run dev -- --port %DEV_PORT% --force"
...
echo  - Ollama:   %VITE_AI_API_BASE% (if enabled)
echo.
echo IMPORTANT - Browser Cache Fix:
echo 1. Wait for "VITE ready" in Frontend window
echo 2. Browser will auto-open to http://localhost:%DEV_PORT%
echo 3. Press Ctrl+Shift+R to FORCE RELOAD (clears cache)
echo 4. Login and test settings dropdown
echo.
